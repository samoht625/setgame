...
export class OnlineMultiplayerSetGame {
    ...
    // Backend URL
    private readonly BACKEND_URL = 'https://setgame-backend.onrender.com';
    ...
    private initializeUI(): void {
        ...
        this.joinRoomButton = document.getElementById('join-room-btn')! as HTMLButtonElement;
        this.createRoomButton = document.getElementById('create-room-btn')! as HTMLButtonElement;
        this.roomCodeInput = document.getElementById('room-code')! as HTMLInputElement;
    ...
    private setupEventListeners(): void {
        ...
        this.roomModal.addEventListener('click', (e) => {
            if (e.target === this.roomModal) this.closeRoomModal();
        });
    ...
    private async connectToRoom(playerName: string): Promise<void> {
        try {
    ...
    private loadGameState(gameState: any): void {
    ...
    private handleCardSelection(cardId: number): void {
    ...
    private updateCards(): void {
        ...
        const cardIds = Array.isArray(displayedCards) ? 
            displayedCards.map(card => typeof card === 'number' ? card : (card as Card).getId()) : 
            [];
        const selectedCardIds = Array.isArray(selectedCards) ? 
            selectedCards.map(card => typeof card === 'number' ? card : (card as Card).getId()) : 
            [];
    ...
    private updateCardsWithDiffing(displayedCards: number[], selectedCards: number[]): void {
    ...
    private createCardElement(cardId: number, isSelected: boolean): HTMLElement {
    ...
    private showActionMessage(text: string, type: 'set_found' | 'player_joined' | 'player_left' | 'game_started' | 'game_ended' | 'error' | 'info'): void {
...
document.addEventListener('DOMContentLoaded', () => {
    (window as any).onlineMultiplayerGame = new OnlineMultiplayerSetGame();
});
